...
def custom_loading_card(text="Memproses data... Mohon tunggu"):
    """Helper component untuk menampilkan animasi loading 3 titik pulse yang estetik"""
    return f"""
    <div class="custom-loader-card">
        ...
        <span style="font-weight: 600; color: #0F172A; font-size: 0.95rem;">{text}</span>
    </div>
    """


def apply_login_styling():
    """CSS custom tambahan untuk merapikan layout background dan card login"""
    css = """
    <style>
    ...
    /* Menyesuaikan padding Streamlit agar pas */
...
def render_metric_card(title, value, delta=None, is_positive=True):
    ...
    if delta:
        delta_class = (
            "metric-delta-positive" if is_positive else "metric-delta-negative"
        )
            ...
            if len(row) >= 2:
                ...
                if email:
                    ...
                    users[email] = {
                        ...
                        "access": {
                            "Project Div": (
                                is_true(row[4]) if len(row) > 4 else False
                            ),
                            "Operation Div": (
                                is_true(row[5]) if len(row) > 5 else False
                            ),
                            "Commercial Div": (
                                is_true(row[6]) if len(row) > 6 else False
                            ),
                            "SCM Div": (
                                is_true(row[7]) if len(row) > 7 else False
                            ),
                            "General / Lapangan": (
                                is_true(row[8]) if len(row) > 8 else False
                            ),
                        },
                    }
        return users
...
def save_password_to_sheet(email_to_update, new_password):
    """Menyimpan/Mengubah Password ke Kolom M (Index 13) di Sheet Otorisasi"""
    try:
        sh = get_google_sheet_connection()
        worksheet = sh.worksheet("Otorisasi")
...
try:
    ...
    show_spk_page = getattr(spk_module, "show_spk_page", None) or getattr(
        spk_module, "render", None
    )
except Exception:
    show_spk_page = None
...
try:
    ...
    show_boq_page = getattr(boq_module, "render", None) or getattr(
        boq_module, "show", None
    )
except Exception:
    show_boq_page = None
...
try:
    ...
    show_scm_page = (
        getattr(scm_module, "render", None)
        or getattr(scm_module, "show_scm_page", None)
        or getattr(scm_module, "show", None)
    )
...
try:
    reimburse_module = importlib.import_module(
        "views.04_commercial.reimbursement"
    )
    show_reimburse_page = (
        getattr(reimburse_module, "render", None)
        or getattr(reimburse_module, "show_reimbursement_page", None)
        or getattr(reimburse_module, "show", None)
    )
...
try:
    invoice_module = importlib.import_module(
        "views.04_commercial.create_invoice"
    )
    show_invoice_page = (
        getattr(invoice_module, "render", None)
        or getattr(invoice_module, "show_invoice_page", None)
        or getattr(invoice_module, "show", None)
    )
...
if not st.session_state["authenticated"]:
    ...
    with col2:
        ...
        st.markdown(
            "<h3 style='text-align: center; color: #1e293b; margin-top: 10px;'>🔒 CLX ERP System</h3>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<p style='text-align: center; color: #64748b; font-size: 14px;'>Silakan masuk dengan akun Anda</p>",
            unsafe_allow_html=True,
        )
        st.write("")

        tab_login, tab_register = st.tabs(
            ["🔐 Login", "🔑 Buat Password Baru"]
        )
        ...
        with tab_login:
            login_email = st.text_input(
                "Email", key="login_email", placeholder=""
            )
            ...
            if btn_login:
                ...
                if email_clean in user_db:
                    ...
                    if not user_info["password"]:
                        st.warning(
                            "⚠️ Email Anda belum memiliki password. Silakan buat password di tab 'Buat Password Baru'."
                        )
        ...
        with tab_register:
            st.caption(
                "Khusus user yang emailnya sudah didaftarkan di Sheet Otorisasi tetapi belum memiliki password."
            )
            ...
            reg_pass1 = st.text_input(
                "Password Baru", type="password", key="reg_pass1"
            )
            reg_pass2 = st.text_input(
                "Konfirmasi Password Baru", type="password", key="reg_pass2"
            )
            btn_register = st.button(
                "Simpan Password", use_container_width=True, type="secondary"
            )
            ...
            if btn_register:
                ...
                if email_clean not in user_db:
                    st.error(
                        "❌ Email Anda belum terdaftar di sheet Otorisasi. Silakan hubungi Admin."
                    )
                    ...
                    # Menggunakan Custom Loading Indicator
                    loader = st.empty()
                    loader.markdown(
                        custom_loading_card(
                            "Menyimpan password ke Google Sheet..."
                        ),
                        unsafe_allow_html=True,
                    )

                    success = save_password_to_sheet(email_clean, reg_pass1)
                    loader.empty()

                    if success:
                        st.success(
                            "✅ Password berhasil disimpan di Sheet Otorisasi! Silakan Login."
                        )
                    else:
                        st.error(
                            "🚨 Gagal memperbarui password di Google Sheets."
                        )
...
allowed_divs = [
    div
    for div, has_access in user_access.items()
    if has_access and div in structure_menu
]
...
with st.sidebar:
    ...
    if allowed_divs:
        ...
        selected_menu = st.radio(
            "Navigasi Menu", structure_menu[selected_div]
        )
    else:
...
if selected_menu == "Create SPK":
    ...
    if show_scm_page:
        ...
        if scm_error_msg:
            st.code(scm_error_msg, language="python")

elif selected_menu == "Create PO":
    st.title("🛍️ Create PO")
    st.info(
        "Modul Create Purchase Order (PO) masih dalam tahap pengembangan."
    )
    ...
    if show_invoice_page:
        ...
        if invoice_error_msg:
            st.code(invoice_error_msg, language="python")
    ...
    if show_reimburse_page:
        ...
        if reimburse_error_msg:
            st.code(reimburse_error_msg, language="python")
